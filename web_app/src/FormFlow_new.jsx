...
// --- КАРТОЧКА ФОРМЫ ---
function FormNode({ id, data, selected, updateNode, deleteNode }) {
...
// --- КАРТОЧКА МЕНЮ ---
function MenuNode({ id, data, selected, updateNode, deleteNode }) {
  const [title, setTitle] = useState(data.label);
  ...
  return (
    <div style={{
      ...
      <div>
        {buttons.map((btn, idx) => (
          <div key={btn.id} style={{ display: 'flex', alignItems: 'center', marginBottom: 4 }}>
            <input
              value={btn.label}
            ...
            <Handle type="source" position={Position.Right} id={`btn-${btn.id}`} style={{ top: 18 + idx * 28, background: '#faad14' }} />
...
function FlowCanvas() {
  ...
  const onDrop = useCallback((event) => {
    ...
    const position = project({
      x: event.clientX - 200, // учесть ширину сайдбара
      y: event.clientY,
    });
    let node;
    if (type === 'form') {
